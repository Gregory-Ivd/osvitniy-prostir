...
(function () {
  ...
  const ADV_TEXTS = [
    "Кожна заява починається з імені, адреси й чіткої мети звернення.",
    "Резюме має вмістити головне на одній сторінці, досвід, навички, контакти.",
    "Сильний пароль - це 12 символів, великі й малі літери, цифри та знаки.",
    "Перед тим як відповідати на лист, перечитай його від початку до кінця.",
    "Хороший документ - це коли читач розуміє суть із першого абзацу, без зайвих слів.",
    "Цифрові навички - це не про техніку, а про впевненість у сучасному житті."
  ];
  ...
  const LESSONS = [
    ...
    { id:"text",  name:"Текст",        hint:"Цілісні речення: великі літери, коми й крапки. Кома — це Shift + крапка.",
      gen:()=>pick(TEXTS) },
    { id:"allrows", name:"Усі ряди",   hint:"Усі рядки разом. Не зупиняйся на помилці — друкуй далі й вирівнюй темп.",
      gen:()=>groups([...HOME,...TOP,...BOT], 10, 2, 5) },
    { id:"advanced", name:"Діловий текст", hint:"Довгі речення з розділовими знаками та великими літерами. Рівний темп важливіший за швидкість.",
      gen:()=>pick(ADV_TEXTS) }
  ];
  ...
  function render(container){
    ...
    function onKey(e){
      ...
      if(got === expected){
        ...
        // пропустити символи без клавіші на розкладці (тире, двокрапка тощо)
        while(state.pos < state.target.length){
          const ch = state.target[state.pos];
          if(FINGER[keyForChar(ch)] !== undefined) break;
          const sp = targetBox.children[state.pos];
          if(sp){ sp.classList.remove("cur"); sp.classList.add("done"); }
          state.pos++;
        }
